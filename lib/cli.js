...
const CLI = {
  ...
  implementation: function implementation() {
    ...
    if (argv.flags.version) {
      ...
      server.start({
        directory: schemaDir,
        silent: argv.flags.silent,
        port: argv.flags.port || process.env.PORT || constants.port,
        fakeroot: argv.flags.fakeroot || constants.fakeroot,
        formats: argv.flags.formats && path.resolve(argv.flags.formats),
      }, err => {
        if (err) {
          this.printError(err);
          process.exit(1);
